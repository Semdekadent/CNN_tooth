...
import pandas as pd
import numpy as np
# import regress_Scan_cross as rg
...
state_dict_list=["best_model_0.pth","best_model_1.pth","best_model_2.pth"]
...
input_tensor = torch.tensor(toothData.values, dtype=torch.long).to(device)
# 进行预测
pre_x = []
predictions = np.zeros((1,input_tensor.shape[0]))
for idx in range(3):
    model.load_state_dict(torch.load(state_dict_list[idx]))
    model.eval()
    with torch.no_grad():
        prediction = model(input_tensor)
        prediction = prediction.view(1,-1).cpu().numpy()
        # print(type(prediction[0][0]))
        predictions=np.concatenate((predictions,prediction),axis=0)
        # idx=np.argmin(prediction)
        # print(f'最优扫描范围是{best_range[0] + 1},扫描误差：{best_range[1]}')
fl_predictions=predictions[1:,:]
fl_predictions=np.mean(fl_predictions,axis=0).tolist()

loaded_scaler = joblib.load('standard_scaler.pkl')

for i, pred in enumerate(fl_predictions):
    pre = loaded_scaler.inverse_transform(np.asarray(fl_predictions[i]).reshape(-1, 1))
    if scan_ranges[i] <= toothData['max_range'][0]:
        predicted_RMS.append(fl_predictions[i])
        pre_x.append(scan_ranges[i])
        print(pre[0][0], end=" ")
    print()
pred_RMS = np.asarray(predicted_RMS)
pred_RMS = loaded_scaler.inverse_transform(pred_RMS.reshape(-1, 1)).flatten()
...
# 创建插值函数
interp_func = interp1d(X, y, kind='cubic')  # 使用三次样条插值
...
plt.scatter(best_range[0], best_range[1], label='best', marker='o', color='black')
plt.plot(X, y, label='pre', linestyle='--', color='blue')
...
# plt.plot(X_interp, y_interp, label='interpolated', linestyle='--', color='blue')
# 获取y轴的数据范围
